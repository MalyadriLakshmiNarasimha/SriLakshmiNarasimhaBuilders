...
import { Menu, X, Home, Info, Building2, Image, BookOpen, Phone, Search as SearchIcon, GitCompare } from 'lucide-react'
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${navbarBg}`}>
      <div className="container-custom">
        <div className="flex justify-between items-center h-20">
          ...
          <div className={`hidden lg:block ml-2 ${textColor}`}>
            <LanguageSwitcher />
          </div>
        ...
        {isOpen && (
          ...
          >
            <div className="container-custom py-4">
              <div className="px-4 py-3 mb-2 text-gray-900"><LanguageSwitcher /></div>
              {navLinks.map((link) => {
                const Icon = link.icon
                const isActive = location.pathname === link.path
                return (
